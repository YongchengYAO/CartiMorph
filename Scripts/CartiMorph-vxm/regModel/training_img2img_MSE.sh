...
# model folder
# e.g. dir_model='~/Documents/CartiMorph/Models_training/vxm/vxm_models/regModel/MSE'
...
# loss function
export imageLoss="mse"

# [Logging] 
export log_file='path/to/log/file/training_img2img_MSE.log' 
...
"$dir_scripts"/train_img2imgRegModel.py --gpu "$gpuIDs" --image-loss "$imageLoss" --enc 16 32 32 32 --dec 32 32 32 32 32 16 16 --img-list "$img_list" --img-prefix "$img_prefix" --img-suffix "$img_suffix" --model-dir "$dir_model" --batch-size "$batch_size"  --epochs "$epochs" --steps-per-epoch "$steps_per_epoch" 2>&1 > "$log_file"
# -----------------------------------------------
