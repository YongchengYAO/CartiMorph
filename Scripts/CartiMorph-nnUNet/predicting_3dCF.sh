...
# e.g. nnUNet_prediction_out='~/Documents/CartiMorph/Models_training/nnUNet/nnUNet_prediction/Task106_test6/inference/3dCF' 
...
# [Logging] 
export log_file='path/to/log/file/predicting_3dCF.log' 

export nnUNet_archi='3d_cascade_fullres' 
...
CUDA_VISIBLE_DEVICES=$gpuIDs CartiMorph_nnUNet_predict -i "$nnUNet_prediction_in" -o "$nnUNet_prediction_out" -f "$nnUNet_fold" -ctr nnUNetTrainerV2CascadeFullRes -m "$nnUNet_archi" -p "$nnUNet_plans_identifier" -t "$nnUNet_taskName" 2>&1 > "$log_file"  

# yes
# CUDA_VISIBLE_DEVICES=$gpuIDs CartiMorph_nnUNet_predict -i "$nnUNet_prediction_in" -o "$nnUNet_prediction_out" -ctr nnUNetTrainerV2CascadeFullRes -m "$nnUNet_archi" -p "$nnUNet_plans_identifier" -t "$nnUNet_taskName" 2>&1 > "$log_file"  
# ----------------------------------------------
