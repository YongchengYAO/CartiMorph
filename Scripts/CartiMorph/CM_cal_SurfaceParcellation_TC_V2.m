function [versSubReg_iC_TC, versSubReg_scB_TC, meshSubReg_iC_TC, meshSubReg_scB_TC, estPA] = ...
    CM_cal_SurfaceParcellation_TC_V2( ...
    mesh_iC_LTC, ...
    mesh_iC_MTC, ...
    mesh_scB_LTC, ...
    mesh_scB_MTC, ...
...
area_scB_MTC = sum(CartiMorphToolbox.quant_triMeshArea(vers_scB_MTC, faces_scB_MTC));
faces_scB_cMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_MTC, vers_scB_MTC, vers_scB_cMTC);
area_scB_cMTC = sum(CartiMorphToolbox.quant_triMeshArea(vers_scB_MTC, faces_scB_cMTC));
...
% adjust the cMTL untill it covers 20% area of the MTL
area_scB_LTC = sum(CartiMorphToolbox.quant_triMeshArea(vers_scB_LTC, faces_scB_LTC));
faces_scB_cLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_LTC, vers_scB_LTC, vers_scB_cLTC);
area_scB_cLTC = sum(CartiMorphToolbox.quant_triMeshArea(vers_scB_LTC, faces_scB_cLTC));
recorder_osillation = 0;
counter_Act = 0;
...
faces_scB_cMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_MTC, subs_scB_MTC, subs_scB_cMTC);
[Mesh_scB_cMTC.vertices, Mesh_scB_cMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_MTC, faces_scB_cMTC);
faces_scB_aMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_MTC, subs_scB_MTC, subs_scB_aMTC);
[Mesh_scB_aMTC.vertices, Mesh_scB_aMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_MTC, faces_scB_aMTC);
faces_scB_pMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_MTC, subs_scB_MTC, subs_scB_pMTC);
[Mesh_scB_pMTC.vertices, Mesh_scB_pMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_MTC, faces_scB_pMTC);
faces_scB_iMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_MTC, subs_scB_MTC, subs_scB_iMTC);
[Mesh_scB_iMTC.vertices, Mesh_scB_iMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_MTC, faces_scB_iMTC);
faces_scB_eMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_MTC, subs_scB_MTC, subs_scB_eMTC);
[Mesh_scB_eMTC.vertices, Mesh_scB_eMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_MTC, faces_scB_eMTC);
faces_scB_cLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_LTC, subs_scB_LTC, subs_scB_cLTC);
[Mesh_scB_cLTC.vertices, Mesh_scB_cLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_LTC, faces_scB_cLTC);
faces_scB_aLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_LTC, subs_scB_LTC, subs_scB_aLTC);
[Mesh_scB_aLTC.vertices, Mesh_scB_aLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_LTC, faces_scB_aLTC);
faces_scB_pLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_LTC, subs_scB_LTC, subs_scB_pLTC);
[Mesh_scB_pLTC.vertices, Mesh_scB_pLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_LTC, faces_scB_pLTC);
faces_scB_iLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_LTC, subs_scB_LTC, subs_scB_iLTC);
[Mesh_scB_iLTC.vertices, Mesh_scB_iLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_LTC, faces_scB_iLTC);
faces_scB_eLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_scB_LTC, subs_scB_LTC, subs_scB_eLTC);
[Mesh_scB_eLTC.vertices, Mesh_scB_eLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_scB_LTC, faces_scB_eLTC);

% original FC
if ~isempty(subs_iC_cMTC)
    faces_iC_cMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_MTC, subs_iC_MTC, subs_iC_cMTC);
    [Mesh_iC_cMTC.vertices, Mesh_iC_cMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_MTC, faces_iC_cMTC);
else
    Mesh_iC_cMTC = [];
end

if ~isempty(subs_iC_aMTC)
    faces_iC_aMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_MTC, subs_iC_MTC, subs_iC_aMTC);
    [Mesh_iC_aMTC.vertices, Mesh_iC_aMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_MTC, faces_iC_aMTC);
else
    Mesh_iC_aMTC = [];
end

if ~isempty(subs_iC_pMTC)
    faces_iC_pMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_MTC, subs_iC_MTC, subs_iC_pMTC);
    [Mesh_iC_pMTC.vertices, Mesh_iC_pMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_MTC, faces_iC_pMTC);
else
    Mesh_iC_pMTC = [];
end

if ~isempty(subs_iC_iMTC)
    faces_iC_iMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_MTC, subs_iC_MTC, subs_iC_iMTC);
    [Mesh_iC_iMTC.vertices, Mesh_iC_iMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_MTC, faces_iC_iMTC);
else
    Mesh_iC_iMTC = [];
end

if ~isempty(subs_iC_eMTC)
    faces_iC_eMTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_MTC, subs_iC_MTC, subs_iC_eMTC);
    [Mesh_iC_eMTC.vertices, Mesh_iC_eMTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_MTC, faces_iC_eMTC);
else
    Mesh_iC_eMTC = [];
end

if ~isempty(subs_iC_cLTC)
    faces_iC_cLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_LTC, subs_iC_LTC, subs_iC_cLTC);
    [Mesh_iC_cLTC.vertices, Mesh_iC_cLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_LTC, faces_iC_cLTC);
else
    Mesh_iC_cLTC = [];
end

if ~isempty(subs_iC_aLTC)
    faces_iC_aLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_LTC, subs_iC_LTC, subs_iC_aLTC);
    [Mesh_iC_aLTC.vertices, Mesh_iC_aLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_LTC, faces_iC_aLTC);
else
    Mesh_iC_aLTC = [];
end

if ~isempty(subs_iC_pLTC)
    faces_iC_pLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_LTC, subs_iC_LTC, subs_iC_pLTC);
    [Mesh_iC_pLTC.vertices, Mesh_iC_pLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_LTC, faces_iC_pLTC);
else
    Mesh_iC_pLTC = [];
end

if ~isempty(subs_iC_iLTC)
    faces_iC_iLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_LTC, subs_iC_LTC, subs_iC_iLTC);
    [Mesh_iC_iLTC.vertices, Mesh_iC_iLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_LTC, faces_iC_iLTC);
else
    Mesh_iC_iLTC = [];
end

if ~isempty(subs_iC_eLTC)
    faces_iC_eLTC = CartiMorphToolbox.cal_extractFaces_OR(faces_iC_LTC, subs_iC_LTC, subs_iC_eLTC);
    [Mesh_iC_eLTC.vertices, Mesh_iC_eLTC.faces] = CartiMorphToolbox.remove_unreferenced_vertices(vers_iC_LTC, faces_iC_eLTC);
else
    Mesh_iC_eLTC = [];
end
...
% estimate posterior to anterior direction
estPA_MTC = transMat_scB_MTC(:, 2); % assuming RAS+ orientation
estPA_LTC = transMat_scB_LTC(:, 2); % assuming RAS+ orientation
estPA = estPA_MTC./norm(estPA_MTC) + estPA_LTC./norm(estPA_LTC);
estPA = estPA ./ norm(estPA);
